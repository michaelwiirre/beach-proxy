...
async function handleLbsfForecast() {
  ...
  try {
    ...
    const tideUrl = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?station=${LBSF_TIDE_STATION}&product=predictions&datum=MLLW&interval=hilo&units=english&time_zone=lst_ldt&format=json&begin_date=${lbsfYmd(today)}&end_date=${lbsfYmd(end)}`;
    const tideRes = await fetch(tideUrl);
    const tideJson = tideRes.ok ? await tideRes.json() : null;
    const hiLoPoints = tideJson?.predictions || [];
    let nightPeriods = [];
    try {
      ...
      if (pointsRes.ok) {
        ...
        if (forecastUrl) {
          const fRes = await fetch(forecastUrl, { headers: { "User-Agent": "LBSF-app (personal fishing tool)" } });
          if (fRes.ok) {
            const fJson = await fRes.json();
            nightPeriods = (fJson?.properties?.periods || []).filter((p) => !p.isDaytime);
          }
        }
      }
    } catch (e) { /* NWS optional */ }

    const nights = [];
    let runningTempAdjust = 0;
    for (let i = 0; i < 7; i++) {
      const d = new Date(today.getTime() + i * 86400000);
      const dateStr = d.toISOString().slice(0, 10);
      const windowStart = lbsfEasternDateTime(dateStr, "20:30:00");
      const windowEnd = lbsfEasternDateTime(dateStr, "23:59:59");
      const relevant = hiLoPoints
        .map((p) => ({ ...p, t: lbsfParseNoaaLocalTime(p.t) }))
        .filter((p) => p.t >= new Date(windowStart.getTime() - 6 * 3600000) && p.t <= new Date(windowEnd.getTime() + 6 * 3600000))
        .sort((a, b) => a.t - b.t);
      let tideDirection = "unknown", tideNote = "";
      if (relevant.length >= 2) {
        const before = relevant.filter((p) => p.t <= windowStart).slice(-1)[0];
        const after = relevant.filter((p) => p.t >= windowStart)[0];
        if (before && after) {
          tideDirection = after.type === "H" ? "incoming" : "outgoing";
          tideNote = `${before.type === "H" ? "high" : "low"} at ${before.t.toTimeString().slice(0, 5)} -> ${after.type === "H" ? "high" : "low"} at ${after.t.toTimeString().slice(0, 5)}`;
        }
      }
      const nwsNight = nightPeriods.find((p) => p.startTime?.slice(0, 10) === dateStr);
      const windSpeed = nwsNight?.windSpeed || null;
      const windDir = nwsNight?.windDirection || null;
      const shortForecast = nwsNight?.shortForecast || null;
      const overnightLowF = nwsNight?.temperature ?? null;
      if (i > 0 && overnightLowF != null && nights[i - 1]?._rawLow != null) {
        const drop = nights[i - 1]._rawLow - overnightLowF;
        if (drop >= 8) runningTempAdjust -= 1.5;
      }
      const waterTempEstF = baseWaterTempF != null ? Math.round((baseWaterTempF + runningTempAdjust) * 10) / 10 : null;
      nights.push({ date: dateStr, tideDirection, tideNote, windSpeed, windDir, shortForecast, waterTempEstF, _rawLow: overnightLowF, isEstimate: i > 0 });
    }
    nights.forEach((n) => delete n._rawLow);
    return new Response(JSON.stringify({ nights, fetchedAt: new Date().toISOString() }), { headers: { "Content-Type": "application/json", ...corsHeaders } });
  } catch (err) {
    return new Response(JSON.stringify({ error: err.message }), { status: 500, headers: { "Content-Type": "application/json", ...corsHeaders } });
  }
}
