...
// ERDDAP). Live-checked and corrected: the original dataset here
// (nesdisVHNSQkd490Daily) is explicitly a "Science Quality" reprocessed
// product with real publication lag, not intended for recent-data
// queries — confirmed by an actual empty response against a real 5-day
// window. Switched to the "Near Real-Time" product instead
// (noaacwNPPVIIRSkd490Daily, on coastwatch.noaa.gov — a different server
// than the Science Quality one), confirmed via real dataset metadata
// (not just one example URL) showing an actual, if still lagged, time
// range. Even NRT shows meaningful lag in practice, so this queries a
// wide 30-day window rather than 5 — resolveSatelliteClarity picks the
// single most recent non-null reading in whatever comes back, and
// separately enforces its own staleness cutoff so a genuinely old
// reading is never presented as if it were current.
// ---------------------------------------------------------------------
...
async function fetchSatelliteClarityRaw(lat, lon, debug = false) {
  // Live-corrected AGAIN: a real ERDDAP error (via the debug endpoint)
  // confirmed the actual bug -- this dataset's real data lags well
  // behind wall-clock "now" (the error showed the axis maximum was
  // already exceeded by a date over 3 weeks before the request time),
  // so computing an ISO end-timestamp from Date.now() was fundamentally
  // the wrong approach: it assumes a real-time-caught-up dataset, which
  // this isn't. ERDDAP has a real, documented feature for exactly this
  // situation -- the literal keyword "last", which always resolves to
  // whatever the dataset's actual most recent available timestep is,
  // whatever that happens to be, without needing to know or guess the
  // real lag. "last-30days" a month before that. This can never produce
  // the "Stop is greater than the axis maximum" error a hardcoded
  // wall-clock timestamp did, regardless of how far behind the dataset
  // ever runs.
  ...
  if (!res.ok) {
    // ERDDAP returns 404 for "no data in this range" (e.g. every pass in
    // the window was cloud-blocked) as well as genuine errors -- these
    // need to be told apart, not both collapsed into the same generic
    // empty response, or there's no way to tell "legitimately no data"
    // from "the query itself is wrong" from the outside.
    let bodyText = null;
...
// ===========================================================================
// PRIORITY 17 (diagnostic only) — real-time USGS turbidity station coverage
// check. Queries USGS's actual, well-documented Site Service (confirmed
// query syntax and error behavior from their real docs, not a single
// example URL) for turbidity-reporting (parameterCd=00076), currently
// live (hasDataTypeCd=iv), active stations within a bounding box covering
// the whole Jacksonville-to-Miami coastline. This does NOT feed the app's
// scoring in any way — it exists purely to answer "does real-time
// turbidity coverage actually exist along this coast" with real data
// before building anything user-facing, given how much guessing the
// satellite integration required.
//
// Note: USGS's Site Service only supports RDB (tab-delimited) output for
// this kind of query, not JSON — this parses that format directly rather
// than reformatting it, since minimal correct-parsing is safer than
// re-deriving a JSON shape by hand for a diagnostic-only route.
// ===========================================================================
function parseUsgsRdb(text) {
  const lines = text.split("\n").filter((l) => l.trim().length > 0 && !l.startsWith("#"));
  if (lines.length < 2) return [];
  const headers = lines[0].split("\t");
  // lines[1] is the format-width line (e.g. "5s 15s 50s..."), not data — skip it
  const rows = [];
  for (let i = 2; i < lines.length; i++) {
...
async function handleUsgsTurbidityCoverage() {
  // Box covering Jacksonville (~30.4N) to Miami (~25.5N) along the FL
  // Atlantic coast, padded slightly. Lat range ~5.5deg x lon range ~1.9deg
  // = ~10.5, comfortably under USGS's 25-degree-product limit.
  const bBox = "-81.6,25.4,-79.7,30.6";
  const url = `https://waterservices.usgs.gov/nwis/site/?format=rdb&bBox=${bBox}&parameterCd=00076&hasDataTypeCd=iv&siteStatus=active&siteOutput=expanded`;
  let res;
  try { res = await fetch(url); }
  catch (e) { return jsonResponse({ error: `Could not reach USGS: ${e.message}`, debugUrl: url }, 502); }
  if (res.status === 404) {
    return jsonResponse({ stationCount: 0, stations: [], note: "USGS returned 404 -- per their own docs, this specifically means no active, real-time, turbidity-reporting sites matched this query in this area.", debugUrl: url });
  }
  if (!res.ok) {
    const bodyText = await res.text().catch(() => null);
    return jsonResponse({ error: `USGS returned HTTP ${res.status}`, debugUrl: url, debugBody: bodyText?.slice(0, 1000) }, 502);
  }
  const text = await res.text();
  const rows = parseUsgsRdb(text);
  const stations = rows.map((r) => ({
    siteNo: r.site_no, name: r.station_nm, lat: r.dec_lat_va ? parseFloat(r.dec_lat_va) : null,
    lon: r.dec_long_va ? parseFloat(r.dec_long_va) : null, siteType: r.site_tp_cd,
  }));
  return jsonResponse({ stationCount: stations.length, stations, debugUrl: url });
}

const FORAGE_DEV_ROUTES = ["/forage/check-sources","/forage/extract-pending","/forage/debug-source","/forage/debug-revision","/forage/review","/forage/review/pending","/forage/review/submit","/forage/review/mark-fully-reviewed","/forage/metrics","/forage/health","/dev/predict-parity","/dev/usgs-turbidity-coverage"];

// ===========================================================================
// LBSF — Cedar Key Shark Predictor routes. Merged from lbsf-worker-routes.js
// verbatim (the user's own already-designed, already-commented logic) —
// nothing rewritten here, only wired into this Worker's existing router
// below. Needs its own KV namespace binding (LBSF_TRIPS) — see the setup
// note at the top of the original lbsf-worker-routes.js file.
// ===========================================================================
async function handleLbsfRequest(request, env) {
  const url = new URL(request.url);
  const corsHeaders = {
    "Access-Control-Allow-Origin": "*",
    "Access-Control-Allow-Methods": "GET, POST, OPTIONS",
  ...
  if (url.pathname === "/api/lbsf/trips" && !env.LBSF_TRIPS) {
    return new Response(JSON.stringify({ error: "Trip storage isn't set up yet — bind an LBSF_TRIPS KV namespace to this Worker in Cloudflare (Settings → Bindings)." }), {
      status: 500, headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  }
  if (url.pathname === "/api/lbsf/trips" && request.method === "POST") {
    const trip = await request.json();
    const key = `trip:${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    await env.LBSF_TRIPS.put(key, JSON.stringify(trip));
    return new Response(JSON.stringify({ ok: true, key }), {
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  }
  if (url.pathname === "/api/lbsf/trips" && request.method === "GET") {
    const list = await env.LBSF_TRIPS.list();
    const trips = await Promise.all(
      list.keys.map(async (k) => {
        const val = await env.LBSF_TRIPS.get(k.name);
        return val ? JSON.parse(val) : null;
      })
    );
    return new Response(JSON.stringify(trips.filter(Boolean)), {
      headers: { "Content-Type": "application/json", ...corsHeaders },
    });
  }
  return null; // not an LBSF route
}

const LBSF_TIDE_STATION = "8727520";       // Cedar Key, FL (NOAA CO-OPS)
const LBSF_WATER_TEMP_STATION = "8727520"; // swap if this station's temp sensor is down

async function lbsfFetchCoOps(station, product, extraParams = "") {
  const url = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?station=${station}&product=${product}&date=latest&units=english&time_zone=lst_ldt&format=json${extraParams}`;
  const res = await fetch(url);
  if (!res.ok) return null;
  return res.json();
}
async function lbsfFetchCoOpsRange(station, product, hours, extraParams = "") {
  const url = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?station=${station}&product=${product}&range=${hours}&units=english&time_zone=lst_ldt&format=json${extraParams}`;
  const res = await fetch(url);
  if (!res.ok) return null;
  return res.json();
}

async function handleLbsfConditions() {
  const corsHeaders = { "Access-Control-Allow-Origin": "*" };
  try {
    const [waterTemp, tideSeries, pressureSeries, wind] = await Promise.all([
      lbsfFetchCoOps(LBSF_WATER_TEMP_STATION, "water_temperature"),
      lbsfFetchCoOpsRange(LBSF_TIDE_STATION, "water_level", 3, "&datum=MLLW"),
      lbsfFetchCoOpsRange(LBSF_TIDE_STATION, "air_pressure", 3),
      lbsfFetchCoOps(LBSF_TIDE_STATION, "wind"),
    ]);
    const waterTempF = waterTemp?.data?.[0]?.v ? parseFloat(waterTemp.data[0].v) : null;
    let tideDirection = null, tideRateFtPerHr = null;
    if (tideSeries?.data?.length >= 2) {
      const pts = tideSeries.data;
      const first = parseFloat(pts[0].v);
      const last = parseFloat(pts[pts.length - 1].v);
      const hoursSpan = pts.length / 12;
      tideRateFtPerHr = (last - first) / hoursSpan;
      tideDirection = tideRateFtPerHr > 0.05 ? "incoming" : tideRateFtPerHr < -0.05 ? "outgoing" : "near slack";
    }
    let pressureTrend = null;
    if (pressureSeries?.data?.length >= 2) {
      const pts = pressureSeries.data;
      const delta = parseFloat(pts[pts.length - 1].v) - parseFloat(pts[0].v);
      pressureTrend = delta < -0.03 ? "falling" : delta > 0.03 ? "rising" : "steady";
    }
    const windSpeed = wind?.data?.[0]?.s ? parseFloat(wind.data[0].s) : null;
    const windDir = wind?.data?.[0]?.dr || null;
    return new Response(JSON.stringify({
      waterTempF, tideDirection, tideRateFtPerHr, pressureTrend, windSpeed, windDir,
...
const LBSF_CEDAR_KEY_LAT = 29.135;
const LBSF_CEDAR_KEY_LON = -83.0317;
function lbsfYmd(d) { return d.toISOString().slice(0, 10).replace(/-/g, ""); }

// Fixes a real bug: the original window-boundary construction below used
// new Date(`${dateStr}T20:30:00`) with no UTC offset. Cloudflare Workers
// run in UTC, so that string parsed as 20:30 UTC — 4:30pm Eastern during
// EDT, not 8:30pm — a constant ~4-5 hour misalignment applied identically
// every day. That's consistent with a real observed symptom: every one of
// 7 different nights scoring identically, since tide direction (worth +20)
// almost never resolved against the wrong window and silently defaulted
// to "unknown" the same way each time. This builds the same date/time
// string but with the correct Eastern offset for that specific date,
// EDT or EST, using the actual US DST rules (2nd Sunday of March to 1st
// Sunday of November) rather than a fixed offset that would itself be
// wrong for half the year.
function lbsfEasternOffsetHours(date) {
  const year = date.getUTCFullYear();
  const marchD = new Date(Date.UTC(year, 2, 1));
  const marchFirstSunday = 1 + ((7 - marchD.getUTCDay()) % 7);
  const dstStart = new Date(Date.UTC(year, 2, marchFirstSunday + 7, 7));
  const novD = new Date(Date.UTC(year, 10, 1));
  const novFirstSunday = 1 + ((7 - novD.getUTCDay()) % 7);
  const dstEnd = new Date(Date.UTC(year, 10, novFirstSunday, 6));
  const isDst = date.getTime() >= dstStart.getTime() && date.getTime() < dstEnd.getTime();
  return isDst ? -4 : -5;
}
function lbsfEasternDateTime(dateStr, timeStr) {
  const approxUtc = new Date(`${dateStr}T${timeStr}Z`); // rough anchor just to determine DST for this date
  const offset = lbsfEasternOffsetHours(approxUtc);
  const abs = Math.abs(offset).toString().padStart(2, "0");
  return new Date(`${dateStr}T${timeStr}-${abs}:00`);
}
// NOAA's time_zone=lst_ldt returns "YYYY-MM-DD HH:MM" already in Cedar
// Key's local Eastern time — semantically correct, but the original code
// (`new Date(p.t.replace(" ", "T"))`, no offset) made a UTC-runtime
// Worker parse it as UTC, the same class of bug as the window boundaries
// above. Both sides of the tide-vs-window comparison need the same fix,
// or they're still being compared inconsistently.
function lbsfParseNoaaLocalTime(noaaTimeStr) {
  const [datePart, timePart] = noaaTimeStr.split(" ");
  return lbsfEasternDateTime(datePart, timePart + ":00");
}

// Ported from the original LBSF.js's computeTideDirections — direction
// and relative strength for EVERY hourly row, by comparing neighbors.
// Much more granular than a single hi/lo boundary check per night, and
// what makes real per-hour "best bite time" scoring possible.
function lbsfComputeTideDirections(tideRows) {
  for (let i = 0; i < tideRows.length; i++) {
    const prevFt = i > 0 ? tideRows[i - 1].tideFt : tideRows[i].tideFt;
    const nextFt = i < tideRows.length - 1 ? tideRows[i + 1].tideFt : tideRows[i].tideFt;
    const delta = nextFt - prevFt;
    tideRows[i].direction = delta >= 0 ? "incoming" : "outgoing";
    tideRows[i].strength = Math.abs(delta);
  }
  return tideRows;
}

async function handleLbsfForecast() {
  const corsHeaders = { "Access-Control-Allow-Origin": "*" };
  try {
    const today = new Date();
    const end = new Date(today.getTime() + 8 * 86400000);
    const tempNow = await lbsfFetchCoOps(LBSF_WATER_TEMP_STATION, "water_temperature");
    const baseWaterTempF = tempNow?.data?.[0]?.v ? parseFloat(tempNow.data[0].v) : null;

    // Hourly tide predictions (not hi/lo) — same interval=h approach as
    // the original script's fetchTidesLocal, correctly Eastern-parsed.
    const tideUrl = `https://api.tidesandcurrents.noaa.gov/api/prod/datagetter?station=${LBSF_TIDE_STATION}&product=predictions&datum=MLLW&interval=h&units=english&time_zone=lst_ldt&format=json&begin_date=${lbsfYmd(today)}&end_date=${lbsfYmd(end)}`;
    const tideRes = await fetch(tideUrl);
    const tideJson = tideRes.ok ? await tideRes.json() : null;
    const tideRows = lbsfComputeTideDirections(
      (tideJson?.predictions || []).map((p) => ({ t: lbsfParseNoaaLocalTime(p.t), tideFt: Number(p.v) }))
    );

    // Hourly NWS forecast (forecastHourly, not the daily/night periods
    // endpoint) — real per-hour wind and short-forecast text.
    let hourlyPeriods = [];
    try {
      const pointsRes = await fetch(`https://api.weather.gov/points/${LBSF_CEDAR_KEY_LAT},${LBSF_CEDAR_KEY_LON}`, { headers: { "User-Agent": "LBSF-app (personal fishing tool)" } });
      if (pointsRes.ok) {
        const pointsJson = await pointsRes.json();
        const hourlyUrl = pointsJson?.properties?.forecastHourly;
        if (hourlyUrl) {
          const fRes = await fetch(hourlyUrl, { headers: { "User-Agent": "LBSF-app (personal fishing tool)" } });
          if (fRes.ok) {
            const fJson = await fRes.json();
            hourlyPeriods = fJson?.properties?.periods || [];
          }
        }
      }
    } catch (e) { /* NWS optional — forecast still works with tide+temp only */ }
    const hourlyWeather = hourlyPeriods.map((p) => ({
      t: new Date(p.startTime).toISOString(),
      windMph: (() => { const m = String(p.windSpeed || "").match(/(\d+)/); return m ? Number(m[1]) : 0; })(),
      shortForecast: p.shortForecast || "",
      overnightLowLikely: p.temperature ?? null,
    }));

    return new Response(JSON.stringify({
      waterTempF: baseWaterTempF,
      tideRows: tideRows.map((r) => ({ t: r.t.toISOString(), tideFt: r.tideFt, direction: r.direction, strength: r.strength })),
      hourlyWeather,
      fetchedAt: new Date().toISOString(),
    }), { headers: { "Content-Type": "application/json", ...corsHeaders } });
  } catch (err) {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/api/lbsf/conditions") return handleLbsfConditions();
    if (url.pathname === "/api/lbsf/forecast") return handleLbsfForecast();
    if (url.pathname === "/api/lbsf/trips") {
      const lbsfResponse = await handleLbsfRequest(request, env);
      if (lbsfResponse) return lbsfResponse;
    }
    ...
    if (url.pathname === "/") {
      return jsonResponse({
        ...
        routes: [
          ...
          "/api/lbsf/conditions", "/api/lbsf/forecast", "/api/lbsf/trips (GET/POST)",
        ],
      });
    }
    ...
    if (url.pathname === "/dev/usgs-turbidity-coverage") return handleUsgsTurbidityCoverage();
