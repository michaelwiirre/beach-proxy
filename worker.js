...
async function fetchSatelliteClarityRaw(lat, lon) {
  const end = new Date();
  ...
  catch (e) { return { error: `Could not reach NOAA CoastWatch ERDDAP: ${e.message}`, status: 502 }; }
  if (!res.ok) {
    // ERDDAP returns 404 for "no data in this range" (e.g. every pass in
    // the window was cloud-blocked) as well as genuine errors — treat as
    // "no reading available" rather than a hard failure either way.
    if (res.status === 404) return { table: { columnNames: ["time", "altitude", "latitude", "longitude", "kd_490"], rows: [] } };
    return { error: `ERDDAP returned HTTP ${res.status}`, status: 502 };
  }
  let json;
  try { json = await res.json(); } catch { return { error: "ERDDAP response was not valid JSON", status: 502 }; }
  return json;
}
async function handleSatelliteClarity(lat, lon) {
  const result = await fetchSatelliteClarityRaw(lat, lon);
  if (result.error) return jsonResponse(result, result.status || 502);
  return jsonResponse(result);
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/clarity/satellite") {
      const lat = url.searchParams.get("lat"), lon = url.searchParams.get("lon");
      if (!lat || !lon || isNaN(+lat) || isNaN(+lon)) return jsonResponse({ error: "Missing or invalid 'lat'/'lon'." }, 400);
      return handleSatelliteClarity(lat, lon);
    }
