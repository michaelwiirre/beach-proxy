...
async function fetchTripsRaw(beachId, env) {
  ...
  try {
    const query = beachId ? "SELECT * FROM trips WHERE beach_id = ? ORDER BY trip_date DESC, id DESC LIMIT 200" : "SELECT * FROM trips ORDER BY trip_date DESC, id DESC LIMIT 200";
    const { results: trips } = beachId ? await env.TRIPS_DB.prepare(query).bind(beachId).all() : await env.TRIPS_DB.prepare(query).all();
