...
// cacheSeconds is optional and backward-compatible: omitted (as in ~95 of
// this file's other call sites — errors, POST endpoints, auth-gated dev
// routes) it behaves exactly as before, no Cache-Control header at all.
// Only the specific read-heavy GET routes below pass it explicitly.
// Requires "cache": { "enabled": true } in this Worker's Wrangler config
// to actually take effect — the header alone does nothing without that.
function jsonResponse(body, status = 200, cacheSeconds = null) {
  const headers = { "Content-Type": "application/json", ...CORS_HEADERS };
  if (cacheSeconds != null && status === 200) headers["Cache-Control"] = `public, max-age=${cacheSeconds}`;
...
async function handleTidesForDate(station, dateCompact) {
  ...
  // Tide predictions are harmonic calculations — deterministic, never
  // change for a given station+date regardless of when queried. Safe to
  // cache far longer than anything observed or forecast.
  return jsonResponse(r.body, r.status, r.status === 200 ? 86400 : null);
}
...
async function handleBuoyHistory(station, dateCompact) {
  ...
  // 10 minutes — NDBC readings update roughly hourly to half-hourly, so
  // this is well inside the real refresh cadence, not stale relative to
  // the source. Not cached indefinitely like tide predictions above:
  // this is a real observation, and "today" can still be accumulating
  // new hourly rows even though the date string itself doesn't change.
  if (rows.length === 0) return jsonResponse({ station, date: dateCompact, rows: [], note: "No data for this date — NDBC's realtime2 file only retains ~45 days, or the buoy may have been offline." }, 200, 600);
  return jsonResponse({ station, date: dateCompact, rows }, 200, 600);
}
...
async function handleWeatherHistory(station, dateCompact) {
  ...
  if (!ok) return jsonResponse({ error }, status);
  return jsonResponse({ station: station.toUpperCase(), date: dateCompact, rows }, 200, 600);
}
...
// Priority 18 — NWS periods forecast (shortForecast text + icon URL per
// day), separate from fetchForecastGridpointRaw above which only pulls
// raw numeric grid layers (temp/wind/precip as time-series values), not
// human-readable condition text. Deliberately a separate function rather
// than folding into the existing one — different NWS endpoint entirely
// (/forecast, the daily periods endpoint, not /forecastGridData), and
// keeping it isolated means this can't affect the already-working
// gridpoint path if something about it needs fixing later.
async function fetchForecastPeriodsRaw(lat, lon) {
  let pointsRes;
  try { pointsRes = await fetch(`https://api.weather.gov/points/${lat},${lon}`, { headers: { "User-Agent": "BeachFishingRadarProxy (personal project)" } }); }
  catch (e) { return { error: `Could not reach NWS points lookup: ${e.message}`, status: 502 }; }
  if (!pointsRes.ok) return { error: `NWS points lookup returned HTTP ${pointsRes.status}`, status: pointsRes.status === 404 ? 404 : 502 };
  ...
  if (!forecastUrl) return { error: "NWS did not return a forecast URL for this location", status: 502 };
  let fRes;
  try { fRes = await fetch(forecastUrl, { headers: { "User-Agent": "BeachFishingRadarProxy (personal project)" } }); }
  catch (e) { return { error: `Could not reach NWS forecast: ${e.message}`, status: 502 }; }
  if (!fRes.ok) return { error: `NWS forecast returned HTTP ${fRes.status}`, status: 502 };
  let fJson;
  try { fJson = await fRes.json(); } catch { return { error: "NWS forecast response was not valid JSON", status: 502 }; }
  const periods = (fJson.properties?.periods || []).map((p) => ({
    startTime: p.startTime, endTime: p.endTime, isDaytime: p.isDaytime,
    shortForecast: p.shortForecast || null, icon: p.icon || null,
    temperature: p.temperature ?? null, windSpeed: p.windSpeed || null, windDirection: p.windDirection || null,
  }));
  return { periods, updated: fJson.properties?.updated || null };
}
async function handleForecastPeriods(lat, lon) {
  const result = await fetchForecastPeriodsRaw(lat, lon);
  if (result.error) return jsonResponse(result, result.status || 502);
  return jsonResponse(result, 200, 600);
}
...
// forecast_days=8 supports the Beach Detail 7-day outlook while preserving the same marine model/source semantics.
// ---------------------------------------------------------------------
...
async function fetchMarineForecastRaw(lat, lon) {
  const qs = new URLSearchParams({
    ...
    models: "meteofrance_wave", timezone: "UTC", forecast_days: "8", cell_selection: "sea",
  });
  let res;
...
// Forecast API. gfs_seamless (not pinned to HRRR alone) — HRRR alone only
// reliably covers ~18h (48h for the 0/6/12/18Z runs), which would leave
// real gaps at the longer horizons this app uses. gfs_seamless uses HRRR
// whenever it's actually available for that hour and GFS otherwise, so
// this stays HRRR-based in spirit for the near term while still covering
// every horizon reliably.
// ---------------------------------------------------------------------
const OPEN_METEO_GFS_BASE = "https://api.open-meteo.com/v1/gfs";
async function fetchWindForecastRaw(lat, lon) {
  const qs = new URLSearchParams({
    latitude: lat, longitude: lon, hourly: "wind_speed_10m,wind_direction_10m",
    models: "gfs_seamless", wind_speed_unit: "kmh", timezone: "UTC", forecast_days: "3",
  });
  let res;
  try { res = await fetch(`${OPEN_METEO_GFS_BASE}?${qs.toString()}`); }
  catch (e) { return { error: `Could not reach Open-Meteo GFS/HRRR API: ${e.message}`, status: 502 }; }
  if (!res.ok) return { error: `Open-Meteo GFS/HRRR API returned HTTP ${res.status}`, status: res.status === 404 ? 404 : 502 };
  let json;
  try { json = await res.json(); } catch { return { error: "Open-Meteo GFS/HRRR API response was not valid JSON", status: 502 }; }
  if (json.error) return { error: json.reason || "Open-Meteo GFS/HRRR API reported an error", status: 400 };
  return json;
}
async function handleWindForecast(lat, lon) {
  const result = await fetchWindForecastRaw(lat, lon);
  if (result.error) return jsonResponse(result, result.status || 502);
  return jsonResponse(result, 200, 600);
}
...
// ERDDAP). Live-debugged across three real corrections, not built on
// guesses: (1) the original dataset was "Science Quality" — reprocessed,
// weeks of lag by design, wrong category for recent-data queries; (2)
// switched to the "Near Real-Time" product (noaacwNPPVIIRSkd490Daily on
// coastwatch.noaa.gov, a different server), but even that runs behind —
// confirmed via a real ERDDAP error, not assumed; (3) that error also
// showed a hardcoded wall-clock end-timestamp is fundamentally wrong for
// a lagged dataset, so this now uses ERDDAP's own "last"/"last-30days"
// keywords, which always resolve to whatever the dataset's real most
// recent data actually is, however far behind that turns out to be.
// A pure supplement to the wave/wind/precip clarity estimate — never a
// replacement — and resolveSatelliteClarity (shared-model.js) enforces
// its own 45-day staleness cutoff so a genuinely old reading is never
// shown as if it describes current water.
// ---------------------------------------------------------------------
const ERDDAP_KD490_BASE = "https://coastwatch.noaa.gov/erddap/griddap/noaacwNPPVIIRSkd490Daily.json";
async function fetchSatelliteClarityRaw(lat, lon, debug = false) {
  const url = `${ERDDAP_KD490_BASE}?kd_490[(last-30days):(last)][(0.0)][(${lat}):(${lat})][(${lon}):(${lon})]`;
  // Real timeout, not just a defensive comment. Satellite clarity is
  // documented everywhere else in this codebase as "a pure supplement,
  // never load-bearing" — but nothing ever actually enforced that until
  // now. A near-identical ~50s delay measured across all 24 beaches
  // (real timing data, not a guess) pointed straight at this third-party
  // ERDDAP call being unconditionally awaited with no time limit at all.
  // 5 seconds is generous for a real response and short enough that a
  // slow/hanging ERDDAP can never meaningfully delay a prediction again.
  ...
  try { res = await fetch(url, { signal: controller.signal }); }
    ...
    const err = { error: isTimeout ? "NOAA CoastWatch ERDDAP timed out after 5s" : `Could not reach NOAA CoastWatch ERDDAP: ${e.message}`, status: 502 };
    return debug ? { ...err, debugUrl: url } : err;
  } finally {
    clearTimeout(timeoutId);
  }
  if (!res.ok) {
    // ERDDAP returns 404 for "no data in this range" as well as genuine
    // errors — these need to be told apart, not both collapsed into the
    // same generic empty response, or there's no way to tell "legitimately
    // no data" from "the query itself is wrong" from the outside.
    let bodyText = null;
    try { bodyText = await res.text(); } catch {}
    if (res.status === 404) {
      const empty = { table: { columnNames: ["time", "altitude", "latitude", "longitude", "kd_490"], rows: [] } };
      return debug ? { ...empty, debugUrl: url, debugStatus: res.status, debugErddapBody: bodyText?.slice(0, 500) } : empty;
    }
    const err = { error: `ERDDAP returned HTTP ${res.status}`, status: 502 };
    return debug ? { ...err, debugUrl: url, debugErddapBody: bodyText?.slice(0, 500) } : err;
  }
  let json;
  try { json = await res.json(); } catch { return { error: "ERDDAP response was not valid JSON", status: 502 }; }
  return debug ? { ...json, debugUrl: url } : json;
}
async function handleSatelliteClarity(lat, lon, debug) {
  const result = await fetchSatelliteClarityRaw(lat, lon, debug);
  if (result.error) return jsonResponse(result, result.status || 502);
  // Never cache a debug call — it needs to hit fresh every time to be a
  // real diagnostic. 1 hour for the normal path: satellite data changes
  // far more slowly than anything else here (see the Priority 16 notes
  // above), so a longer TTL doesn't risk meaningful staleness.
  return jsonResponse(result, 200, debug ? null : 3600);
}
...
function requireForageAuth(request, env) {
  ...
  // Safari's URL bar can't set custom headers, so a visited-directly URL
  // (like the diagnostic routes) needs a fallback — a ?token= query param,
  // checked only if no valid header was already provided.
  const url = new URL(request.url);
  const queryToken = url.searchParams.get("token");
  const token = headerToken || queryToken;
  if (!env.FORAGE_DEV_TOKEN || token !== env.FORAGE_DEV_TOKEN) {
    return jsonResponse({ error: "Unauthorized. Pass 'Authorization: Bearer <FORAGE_DEV_TOKEN>' or add '?token=<FORAGE_DEV_TOKEN>' to the URL." }, 401);
  }
  return null;
}
...
function buildWorkerDataAccess(env) {
  ...
  return {
    ...
    fetchWindForecastForBeach: (beach) => cached(`wind:${beach.id}`, () => fetchWindForecastRaw(beach.lat, beach.lon)),
    fetchSatelliteClarityForBeach: (beach) => cached(`satclarity:${beach.id}`, () => fetchSatelliteClarityRaw(beach.lat, beach.lon)),
...
// ===========================================================================
// PRIORITY 15 STAGE 3 — immutable forecast issuance + later verification.
// Diagnostic only: this records forecast error; it never changes scoring.
// ===========================================================================
const FORECAST_VERIFY_HORIZONS_H = [1, 3, 6, 12, 18, 24];
const FORECAST_VERIFY_SPECIES = ["pompano","blacktip","spinner","bull","tarpon","snook","jack","bluefish","whiting","mackerel"];
const FORECAST_VERIFY_BEACHES = [
  ["amelia-island",30.6714,-81.4551],["jacksonville-beach",30.2947,-81.3931],["ponte-vedra-beach",30.2394,-81.3862],
  ["st-augustine-beach",29.8508,-81.2648],["vilano-beach",29.9139,-81.3081],["flagler-beach",29.4747,-81.1256],
  ["playalinda-beach",28.6197,-80.6081],["new-smyrna-beach",29.0258,-80.9270],["daytona-beach",29.2108,-81.0228],
  ["cocoa-beach",28.3200,-80.6076],["melbourne-beach",28.0798,-80.5623],["vero-beach",27.6386,-80.3973],
  ["fort-pierce",27.4762,-80.3078],["jensen-beach",27.2515,-80.2231],["bathtub-beach",27.1706,-80.2298],
  ["stuart-beach",27.1739,-80.1917],["juno-beach",26.8792,-80.0553],["jupiter-beach",26.9342,-80.0731],
  ["palm-beach",26.7056,-80.0364],["fort-lauderdale-beach",26.1224,-80.1039],["hollywood-beach",26.0112,-80.1187],
  ["haulover-beach",25.9026,-80.1211],["south-beach",25.7826,-80.1341],["key-biscayne",25.6931,-80.1625],
].map(([id,lat,lon]) => ({id,lat,lon}));

function stage3ResultPayload(envelope) {
  const r = envelope?.result, e = envelope?.environmentalInputs || {};
  return {
    presence: r?.presence ?? null, feeding: r?.feeding ?? null, access: r?.access ?? null,
    finalScore: r?.score ?? null, scoreRaw: r?.scoreRaw ?? null, confidence: r?.confidence ?? null,
    position: r?.position ?? null, factors: r?.factors ?? null,
    environmentalInputs: e, sourceMeta: envelope?.sourceMeta ?? null,
    forecastProvenance: envelope?.forecastProvenance ?? null,
  };
}

async function issueForecastVerificationBatch(env, issuedAt = new Date()) {
  if (!env.TRIPS_DB) return { ok:false, error:"TRIPS_DB binding missing" };
  const dataAccess = buildWorkerDataAccess(env); // exactly once per issuance
  const issuanceKey = issuedAt.toISOString().slice(0,13); // idempotent within the cron hour
  let inserted=0, skipped=0, failed=0;
  for (const beach of FORECAST_VERIFY_BEACHES) for (const speciesId of FORECAST_VERIFY_SPECIES) for (const horizonH of FORECAST_VERIFY_HORIZONS_H) {
    const target = new Date(issuedAt.getTime() + horizonH*3600000);
    try {
      const envelope = await predictAsOfTimestamp(beach, speciesId, target, dataAccess);
      if (!envelope?.result) { skipped++; continue; }
      const x = stage3ResultPayload(envelope);
      const res = await env.TRIPS_DB.prepare(`INSERT OR IGNORE INTO forecast_verification_snapshots
        (issuance_key,beach_id,lat,lon,species_id,issued_at,target_timestamp,horizon_hours,model_version,implementation_fingerprint,
         predicted_presence,predicted_feeding,predicted_access,predicted_final_score,predicted_score_raw,predicted_confidence,
         predicted_environment_json,predicted_source_meta_json,forecast_provenance_json,predicted_position_json,predicted_factors_json)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(issuanceKey,beach.id,beach.lat,beach.lon,speciesId,issuedAt.toISOString(),target.toISOString(),horizonH,
          MODEL_VERSION,MODEL_IMPLEMENTATION_FINGERPRINT,x.presence,x.feeding,x.access,x.finalScore,x.scoreRaw,x.confidence,
          JSON.stringify(x.environmentalInputs),JSON.stringify(x.sourceMeta),JSON.stringify(x.forecastProvenance),JSON.stringify(x.position),JSON.stringify(x.factors)).run();
      if ((res.meta?.changes || 0) > 0) inserted++; else skipped++;
    } catch (e) { failed++; console.error("stage3 issue failed", beach.id, speciesId, horizonH, e); }
  }
  return {ok:true,inserted,skipped,failed,issuedAt:issuedAt.toISOString()};
}

async function verifyDueForecastSnapshots(env, verifiedAt = new Date()) {
  if (!env.TRIPS_DB) return { ok:false, error:"TRIPS_DB binding missing" };
  const {results=[]} = await env.TRIPS_DB.prepare(`SELECT s.* FROM forecast_verification_snapshots s
    LEFT JOIN forecast_verification_results v ON v.snapshot_id=s.id
    WHERE v.snapshot_id IS NULL AND s.target_timestamp <= ? ORDER BY s.target_timestamp ASC LIMIT 500`).bind(verifiedAt.toISOString()).all();
  const dataAccess = buildWorkerDataAccess(env); // exactly once per verification batch
  let verified=0, excluded=0, failed=0;
  for (const s of results) {
    const versionMatch = s.model_version === MODEL_VERSION && s.implementation_fingerprint === MODEL_IMPLEMENTATION_FINGERPRINT;
    if (!versionMatch) {
      await env.TRIPS_DB.prepare(`INSERT OR IGNORE INTO forecast_verification_results
        (snapshot_id,verified_at,verification_delay_minutes,eligible_for_metrics,exclusion_reason,actual_model_version,actual_implementation_fingerprint)
        VALUES (?,?,?,?,?,?,?)`).bind(s.id,verifiedAt.toISOString(),Math.round((verifiedAt-new Date(s.target_timestamp))/60000),0,
          "model_or_implementation_mismatch",MODEL_VERSION,MODEL_IMPLEMENTATION_FINGERPRINT).run(); excluded++; continue;
    }
    try {
      // Replay the TARGET timestamp after it has passed. This uses historical observations at the target,
      // rather than conditions at cron execution time, so cron delay does not contaminate forecast error.
      const beach={id:s.beach_id,lat:s.lat,lon:s.lon};
      const envelope=await predictAsOfTimestamp(beach,s.species_id,new Date(s.target_timestamp),dataAccess);
      if (!envelope?.result) { failed++; continue; }
      const x=stage3ResultPayload(envelope), delay=Math.round((verifiedAt-new Date(s.target_timestamp))/60000);
      await env.TRIPS_DB.prepare(`INSERT OR IGNORE INTO forecast_verification_results
        (snapshot_id,verified_at,verification_delay_minutes,eligible_for_metrics,actual_model_version,actual_implementation_fingerprint,
         actual_presence,actual_feeding,actual_access,actual_final_score,actual_score_raw,actual_confidence,
         score_error,presence_error,feeding_error,access_error,actual_environment_json,actual_source_meta_json,actual_position_json,actual_factors_json)
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
        .bind(s.id,verifiedAt.toISOString(),delay,1,MODEL_VERSION,MODEL_IMPLEMENTATION_FINGERPRINT,
          x.presence,x.feeding,x.access,x.finalScore,x.scoreRaw,x.confidence,
          x.finalScore-s.predicted_final_score,x.presence-s.predicted_presence,x.feeding-s.predicted_feeding,x.access-s.predicted_access,
          JSON.stringify(x.environmentalInputs),JSON.stringify(x.sourceMeta),JSON.stringify(x.position),JSON.stringify(x.factors)).run(); verified++;
    } catch(e) { failed++; console.error("stage3 verify failed",s.id,e); }
  }
  return {ok:true,due:results.length,verified,excluded,failed,verifiedAt:verifiedAt.toISOString()};
}

async function handleForecastVerificationMetrics(url, env) {
  if (!env.TRIPS_DB) return jsonResponse({error:"TRIPS_DB binding missing"},500);
  const minN=Math.max(1,parseInt(url.searchParams.get("min_n")||"10",10)||10);
  const {results=[]}=await env.TRIPS_DB.prepare(`SELECT s.horizon_hours, COUNT(*) n,
    ROUND(AVG(ABS(v.score_error)),2) mae_score, ROUND(AVG(v.score_error),2) bias_score,
    ROUND(AVG(ABS(v.presence_error)),2) mae_presence, ROUND(AVG(ABS(v.feeding_error)),2) mae_feeding,
    ROUND(AVG(ABS(v.access_error)),2) mae_access
    FROM forecast_verification_results v JOIN forecast_verification_snapshots s ON s.id=v.snapshot_id
    WHERE v.eligible_for_metrics=1 GROUP BY s.horizon_hours ORDER BY s.horizon_hours`).all();
  return jsonResponse({modelVersion:MODEL_VERSION,implementationFingerprint:MODEL_IMPLEMENTATION_FINGERPRINT,minN,
    horizons:results.map(r=>({...r,reliable:r.n>=minN}))});
}

async function forecastVerificationScheduledHandler(env) {
  // Verify first so a just-due target is evaluated before new issuance work starts.
  const verification=await verifyDueForecastSnapshots(env,new Date());
  const issuance=await issueForecastVerificationBatch(env,new Date());
  console.log("Priority 15 Stage 3",JSON.stringify({verification,issuance}));
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/forecast/periods") {
      const lat = url.searchParams.get("lat"), lon = url.searchParams.get("lon");
      if (!lat || !lon || isNaN(+lat) || isNaN(+lon)) return jsonResponse({ error: "Missing or invalid 'lat'/'lon'." }, 400);
      return handleForecastPeriods(lat, lon);
    }
    ...
    if (url.pathname === "/wind/forecast") {
      const lat = url.searchParams.get("lat"), lon = url.searchParams.get("lon");
      if (!lat || !lon || isNaN(+lat) || isNaN(+lon)) return jsonResponse({ error: "Missing or invalid 'lat'/'lon'." }, 400);
      return handleWindForecast(lat, lon);
    }
    if (url.pathname === "/clarity/satellite") {
      const lat = url.searchParams.get("lat"), lon = url.searchParams.get("lon");
      if (!lat || !lon || isNaN(+lat) || isNaN(+lon)) return jsonResponse({ error: "Missing or invalid 'lat'/'lon'." }, 400);
      return handleSatelliteClarity(lat, lon, url.searchParams.get("debug") === "1");
    }
    ...
    if (url.pathname === "/forecast-verification/metrics") return handleForecastVerificationMetrics(url, env);

    return jsonResponse({ error: "Unknown route." }, 404);
  },

  async scheduled(event, env, ctx) {
    ctx.waitUntil(Promise.all([forageScheduledHandler(env), forecastVerificationScheduledHandler(env)]));
  },
};
