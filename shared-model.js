...
// NOAA CO-OPS tide requests are calendar-day based. Florida Atlantic-coast
// stations use Eastern local time, so derive the request day explicitly in
// America/New_York rather than from UTC. Keep dateToCompact() UTC because
// NDBC/METAR history files are UTC-oriented.
...
export function normalizeNoaaTideRows(predictions, timeZone = "America/New_York") {
  return (predictions || []).map((p) => {
    // Priority 16 timezone correctness: the prediction-history path requests
    // NOAA in GMT, so its timestamps can be made absolute without relying on
    // the browser/Worker runtime timezone. The Eastern fallback is retained
    // for legacy/local NOAA payloads, but history callers should pass "UTC".
    let observedAtUtc = null;
    if (timeZone === "UTC" || timeZone === "gmt") {
      const iso = String(p.t || "").trim().replace(" ", "T");
      const ms = Date.parse(`${iso}:00Z`);
      if (Number.isFinite(ms)) observedAtUtc = new Date(ms).toISOString();
    }
    return { time: p.t, observedAtUtc, value: parseFloat(p.v), tideType: p.type === "H" ? "H" : "L" };
  });
}
...
export function normalizeObservations({ rows, stationId, stationName, product, parameter, unit, datum, sourceType, reliability, retrievedAt }) {
  return rows.map((r) => ({
    ...
    observedAt: r.time, // raw NOAA timestamp retained for provenance/display
    observedAtUtc: r.observedAtUtc || null, // absolute instant when available (history path requests GMT)
    retrievedAt,
...
export function computeTideStage(hiloRows, atDate) {
  // Never parse NOAA's timezone-naive station-local string here. History
  // normalization supplies observedAtUtc from a GMT NOAA response, making
  // browser and Worker comparisons deterministic across runtime timezones.
  const tideMs = (row) => row?.observedAtUtc ? Date.parse(row.observedAtUtc) : NaN;
  const sorted = [...hiloRows].filter((r) => Number.isFinite(tideMs(r))).sort((a, b) => tideMs(a) - tideMs(b));
  const atMs = atDate.getTime();
  for (let i = 0; i < sorted.length - 1; i++) {
    const prev = sorted[i], next = sorted[i + 1];
    const prevMs = tideMs(prev);
    const nextMs = tideMs(next);
  ...
  return null; // target falls outside the fetched prediction window
}
...
export async function predictAsOfTimestamp(beach, speciesId, predictionTimestamp, dataAccess) {
  ...
  const tideDateCompactStr = dateToEasternCompact(predictionTimestamp);
  ...
  const sourceMeta = { tide: "unavailable", buoy: "unavailable", precip: "unavailable", wave: "unavailable" };
  ...
  if (noaaStation) {
    try {
      tideRows = await dataAccess.fetchTideHistoryDay(noaaStation, tideDateCompactStr);
...
export const MODEL_IMPLEMENTATION_FINGERPRINT = "aace38a3bb38a936";
