...
// Same YYYYMMDD compact format, but the Eastern LOCAL calendar date
// rather than the UTC calendar date. Needed near midnight Eastern,
// where the UTC calendar date is already the next day — the same class
// of bug fixed for LBSF's tide-window matching earlier. Uses Intl's own
// timezone conversion (DST-aware automatically) rather than a
// hand-rolled offset calculation.
export function dateToEasternCompact(d) {
...
// Defensive parsing, live-corrected after a real empty response revealed
// the original dataset was wrong (see worker.js comment). Takes the most
// recent non-null value in whatever window was fetched, since cloud
// cover blanks out individual days — but also enforces its own
// staleness cutoff (10 days) separate from however wide a window the
// caller queried: even Near-Real-Time satellite products can lag for
// weeks in practice, and a reading that old shouldn't be presented as
// if it describes current water conditions. Beyond that, this honestly
// returns unavailable rather than a misleadingly stale "supplement."
export function resolveSatelliteClarity(erddapJson, requestTimestamp) {
  try {
    ...
    if (!best) return null;
    const ageHoursCheck = (requestTimestamp.getTime() - best.t.getTime()) / 3600000;
    // Widened from an initial 10-day guess after a real ERDDAP error
    // (see worker.js) confirmed this specific dataset's actual lag is
    // worse than that in practice — the axis maximum was already
    // exceeded by a date over 3 weeks before the request. 45 days is
    // still a real bound, not "anything goes": a reading older than
    // that genuinely shouldn't be called a description of current water.
    if (ageHoursCheck < 0 || ageHoursCheck > 1080) return null; // older than 45 days -- too stale to call "current"
...
export async function predictAsOfTimestamp(beach, speciesId, predictionTimestamp, dataAccess) {
  ...
  // Performance — these two have zero dependency on anything computed
  // below (satellite clarity and trip history are both purely beach-
  // scoped, never touch tide/buoy/forecast), so their fetches start
  // here, immediately, running concurrently with everything else in this
  // function. They're only actually awaited later at their existing
  // usage points — same results, same order of processing, same
  // defensive error handling as before, just no longer forced to wait
  // for every other fetch to finish first before even starting.
  const satelliteClarityPromise = dataAccess.fetchSatelliteClarityForBeach(beach).catch(() => null);
  const tripsPromise = dataAccess.fetchTripsList(beach.id); // left rejectable — the existing try/catch at its usage point below must still see a real rejection to preserve its exact original fallback behavior (skip forage-evidence entirely), not a fabricated empty result
  // Tide predictions are also independent of the isFuture/historical
  // branch below (that branch only ever reads/sets buoyReading and
  // precipReading — tideRows never feeds into it, and vice versa), so
  // this starts here too rather than only once the branch below finishes.
  const tideRowsPromise = noaaStation ? dataAccess.fetchTideHistoryDay(noaaStation, dateCompactStr) : null;

  let tideRows = null;
  if (noaaStation) {
    try {
      tideRows = await tideRowsPromise;
  ...
  if (isFuture) {
    // Performance — forecastBundle and marineBundle have no dependency on
    // each other's RESULT (marineBundle always runs and always overwrites
    // wave height regardless of what forecastBundle found), only
    // windBundle below genuinely depends on forecastBundle's outcome. So
    // both fetches start here, concurrently; each result is still
    // processed in the exact same order as before (forecast first, then
    // marine) — only the network requests themselves now overlap instead
    // of running one after the other.
    const forecastBundlePromise = dataAccess.fetchForecastForBeach(beach);
    const marineBundlePromise = dataAccess.fetchMarineForecastForBeach(beach);
    ...
    try {
      const forecastBundle = await forecastBundlePromise;
    ...
    try {
      const marineBundle = await marineBundlePromise;
  ...
  try {
    const erddapJson = await satelliteClarityPromise;
...
export const MODEL_IMPLEMENTATION_FINGERPRINT = "fc3b50c99a365ef4";
